...
def read_input_field(expr: Union[str, float, int], V: FunctionSpace = None, mesh=None):
    if V is None: return float(expr)
    try:
    ...
    if isinstance(expr, (float, int)):
        if V is None:
            return float(expr)
        return Expression(ufl.as_ufl(expr), interp_points, MPI.COMM_WORLD)
        ...
        return Expression(ufl_expr, interp_points, MPI.COMM_WORLD) if V else ufl_expr
